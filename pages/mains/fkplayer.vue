<template>
	...
	<view>
		<view class="cu-bar bg search bg-gradual-blue fixed">
			...
			<view class=" title-text text-center text-xl" >我的房卡玩家</view>
			<view class="action">
			</view>
		</view>
		...
		<view class="cu-list menu padding-xl radius shadow-warp bg-white " style="border-radius:4upx ;" :class="[0?'sm-border':'',1?'card-menu ':'']" v-for="(pd,k) in curPageData" :key="k">
			<view class=""  @tap="changeSN(k)">
				<view class="cu-item" :class="pd.isDisplay?'bg-grey':'bg-white'">
					<view class="flex">
						<view class="flex-sub  padding-sm  radius">
							<text class="list-text3">日期:</text>
							<text class="list-text7">{{pd.registerTime}}</text>
						</view>
						<view class="flex-sub  padding-sm  radius ">
							<text class="list-text4">玩家ID:</text>
							<text class="list-text3">{{pd.userid}}</text>
						</view>
					</view>
				</view>
				<view class="cu-item " :class="pd.isDisplay ? 'bg-c bg-gray':'bg-white'" >
					<view class="flex">
						<view class="flex-sub  padding-sm  radius ">
							<text class="list-text3">钻石数量:</text>
							<text class="list-text3">{{pd.diamond}}</text>
						</view>
						<view class="flex-sub  padding-sm  radius">
							<text class="list-text3">绑钻数量:</text>
							<text class="list-text3">{{pd.bind_diamond}}</text>
						</view>
					</view>
				</view>
			</view>
			<view :class="pd.isDisplay ? 'show bg-c bg-gray solids-top' : 'hide bg-white'">
				...
				<view class="flex text">
					<view class="flex-sub  padding-sm  radius">
						<text class="list-text3">昵称:</text>
						<text class="list-text3">{{pd.nickName}}</text>
					</view>
					
					<view class="flex-sub  padding-sm  radius">
						<text class="list-text3">房卡绑定ID:</text>
						<text class="list-text3">{{pd.room_agent_id}}</text>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			upCallback(mescroll) {
				...
				uni.request({
					url:helper.websiteUrl+'/agent/tuiguang/room-player-list',
					header: {
