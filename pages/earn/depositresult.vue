<template>
	<view>
		<view class="cu-bar bg search bg-gradual-blue fixed">
			<view class="action" @tap="navTo()"><text class="cuIcon-back text-white"></text>返回</view>
		...
		<view class="bg-white nav text-center fixed" :style="[{ top: CustomBar + 'px' }]">
			...
			<view class="cu-bar search bg-white hander1">
				...
				<view class="action"><button class="cu-btn bg-gradual-blue shadow-blur round" :disabled="isDisable4" @click="loadlist()">查询</button></view>
			</view>
		</view>
		...
		<mescroll-uni top="300" bottom="50" @down="downCallback" @up="upCallback" @init="mescrollInit">
			<!-- 分页的数据列表 -->
			<view
				class="cu-list menu padding-xl radius shadow-warp bg-white "
				style="border-radius:4upx ;"
				:class="[0 ? 'sm-border' : '', 1 ? 'card-menu ' : '']"
				v-for="(pd, k) in curPageData"
				:key="k">
				<view class="" @tap="changeSN(k)">
					<view class="cu-item" :class="pd.isDisplay ? 'bg-c bg-grey' : 'bg-white'">
						<view class="flex">
							...
							<view class="flex-sub  padding-sm  radius ">
								...
								<text class="list-text3">{{ pd.deposit_money }}</text>
							</view>
							<view class="  padding-sm  radius"><text class="cuIcon-fold text-blue icon-title" :class="pd.isDisplay ? 'show' : 'hide'"></text></view>
						</view>
					</view>
					<view class="cu-item " :class="pd.isDisplay ? 'bg-c bg-gray' : 'bg-white'">
						<view class="flex">
							...
							<view class="  padding-sm  radius"><text class="cuIcon-unfold text-blue icon-title" :class="pd.isDisplay ? ' hide' : 'show'"></text></view>
						</view>
					</view>
				</view>
				...
				<view :class="pd.isDisplay ? 'show bg-c bg-gray solids-top' : 'hide bg-white'">
					<view class="flex">
						...
						<view class="flex-sub  padding-sm  radius">
							<text class="list-text2">提现人:</text>
							<text class="list-text3">{{ pd.account_name }}</text>
						</view>
					</view>
					<view class="flex text">
						<view class="flex-sub  padding-sm  radius text1" >
							<text class="list-text2">审核说明:</text>
							<text class="list-text3 result">{{ pd.check_remark }}</text>
						</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		upCallback(mescroll) {
			...
			uni.request({
				url: helper.websiteUrl + '/agent/earnings/ajax-deposit-list',
				header: {
					...
					if (data.code == 200) {
						...
						for (let i = 0; i < listdata.length; i++) {
							...
							if (parseInt(deposit_type) == 0) {
								...
								listdata[i].deposit_type = '中国银行';
							} else if (parseInt(deposit_type) == 3) {
								listdata[i].deposit_type = '招商银行';
							} else if (parseInt(deposit_type) == 4) {
								listdata[i].deposit_type = '建设银行';
							} else if (parseInt(deposit_type) == 5) {
								listdata[i].deposit_type = '农业银行';
							} else if (parseInt(deposit_type) == 6) {
								listdata[i].deposit_type = '工商银行';
							}
							...
							if (parseInt(status) == 0) {
								...
								listdata[i].status = '待处理';
							}
							let check_remark=listdata[i].check_remark
							if(check_remark==""|| check_remark==null){
								listdata[i].check_remark="无"
							}
						}
